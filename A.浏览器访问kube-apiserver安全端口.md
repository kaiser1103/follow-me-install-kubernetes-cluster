...
再次访问 https://192.168.112.100:6443/，已信任，但提示 401，未授权的访问：
...
``` bash
[k8s@master ~]$ openssl pkcs12 -export -out admin.pfx -inkey admin-key.pem -in admin.pem -certfile ca.pem
```
...
**重启浏览器**，再次访问 https://192.168.112.100:6443/，提示选择一个浏览器证书，这里选中上面导入的 admin.pfx：
