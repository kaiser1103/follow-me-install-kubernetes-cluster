...
- [10.部署私有 docker registry](#10部署私有-docker-registry)
...
health:
  storagedriver:
    ...
    threshold: 3
EOF
[k8s@master cert]$ cp config.yml registry
[k8s@master cert]$ scp -r registry 172.27.132.67:/opt/k8s
```
...
``` bash
[k8s@master cert]$ sudo mkdir -p /etc/docker/certs.d/172.27.132.67:8000
[k8s@master cert]$ sudo cp /etc/kubernetes/cert/ca.pem /etc/docker/certs.d/172.27.132.67:8000/ca.crt
```
...
[k8s@master ~]$  rados --pool default.rgw.buckets.data ls|grep node-exporter
...
``` bash
[k8s@master ~]$ curl  --user foo:foo123 --cacert /etc/docker/certs.d/172.27.132.67\:8000/ca.crt https://172.27.132.67:8000/v2/_catalog
...
``` bash
[k8s@master ~]$  curl  --user foo:foo123 --cacert /etc/docker/certs.d/172.27.132.67\:8000/ca.crt https://172.27.132.67:8000/v2/prom/node-exporter/tags/list
...
``` bash
[k8s@master ~]$ curl -v -H "Accept: application/vnd.docker.distribution.manifest.v2+json" --user foo:foo123 --cacert /etc/docker/certs.d/172.27.132.67\:8000/ca.crt https://172.27.132.67:8000/v2/prom/node-exporter/manifests/v0.16.0
...
[k8s@master cert]$ python s3test.py
...
1. ceph-deploy config push master node01 node02
1.  systemctl restart 'ceph-mds@node02.service'
    systemctl restart ceph-osd@0
    systemctl restart 'ceph-mon@master.service'
    systemctl restart 'ceph-mgr@master.service'
...
[root@master ~]# docker login  172.27.132.67:8000
Username: foo
Password:
