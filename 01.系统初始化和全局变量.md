...
``` bash
cat >> /etc/hosts <<EOF
192.168.112.100 master	master
192.168.112.101 node01	node01
192.168.112.102 node02	node02
EOF
```
...
如果没有特殊指明，本文档的所有操作**均在 master 节点上执行**，然后远程分发文件和执行命令，所以需要添加该节点到其它节点的 ssh 信任关系。

设置 master 可以无密码登录**所有节点**的 root 账户：
