...
注意：
    1.如果没有特殊指明，本文档的所有操作**均在 master 节点上执行**，然后远程分发文件和执行命令。
    2.该节如果没有分别部署，会在另外两个节点上面kubectl无法使用（apiserver-address 都会是 127.0.0.1:8443），所以该节需要在所有节点部署。
...
stream {
    upstream backend {
        ...
        server 192.168.112.100:6443          max_fails=3 fail_timeout=30s;
        server 192.168.112.101:6443         max_fails=3 fail_timeout=30s;
        server 192.168.112.102:6443        max_fails=3 fail_timeout=30s;
    }
