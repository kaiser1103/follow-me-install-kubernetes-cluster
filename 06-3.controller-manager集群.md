...
kubectl config use-context kube-controller-manager --kubeconfig=kube-controller-manager.kubeconfig
```
...
ExecStart=/opt/k8s/bin/kube-controller-manager \\
  ...
  --requestheader-client-ca-file=/etc/kubernetes/cert/kube-controller-manager.pem \\
...
## **kaiser备注：**
    ...
    这里经过测试，不像是因为没有部署metrics-server的缘故，
    我把master节点的/opt/k8s/work/{ca.pem,admin.pem,admin-key.pem}三个证书放在node节点上，
    再执行：
    
    curl --cacert /opt/k8s/work/ca.pem --cert /opt/k8s/work/admin.pem --key /opt/k8s/work/admin-key.pem https://127.0.0.1:10252/metrics
    
    就可以看到和master一样的结果，但是用证书/etc/kubernetes/cert/{ca.pem,admin.pem,admin-key.pem}就不行。
    不知道为什么证书效果不一样。
...
metadata:
  annotations:
    control-plane.alpha.kubernetes.io/leader: '{"holderIdentity":"node01_084534e2-6cc4-11e8-a418-5254001f5b65","leaseDurationSeconds":15,"acquireTime":"2018-06-10T15:40:33Z","renewTime":"2018-06-10T16:19:08Z","leaderTransitions":12}'
